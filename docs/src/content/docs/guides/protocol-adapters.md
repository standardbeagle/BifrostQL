...
- `AdapterSupportsMalformedFrameProbe` — the adapter owns a connection handler that decodes an unauthenticated peer's bytes. The derivation overrides `ProbeMalformedFrameAsync`, running one connection of its real handler over the scripted wire the kit supplies; the kit drives a fixed corpus (empty, NUL, truncated length prefix, high bytes, printable garbage, deeply nested aggregate and BER headers, and five pinned PRNG seeds) and asserts no frame escapes the handler's catch filter and none leaves the connection hanging.
- `AdapterSupportsTlsAdmissionProbe` — the adapter terminates TLS on its own listener, so the *order* of admission and handshake is its to get right. The derivation overrides `ProbeTlsAdmissionAsync`, binding a real port with a real certificate at a cap of one connection; the kit asserts a peer that connects and sends nothing still costs a slot, and that the next peer is closed without completing a handshake. The probe has to bind a port because the ordering under test is the listener's connection-middleware registration order, which no in-process stream harness can observe.
...
- [ ] Admitted-session lifecycle routed through `ProtocolSessionHost` — never a private admission or pre-auth-deadline copy.
