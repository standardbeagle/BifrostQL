...
import { AttendanceByMemberReport } from './attendance-by-member-report';
...
function renderReport() {
  return render(
    <PathProvider path="/reports/attendance-by-member">
...
describe('AttendanceByMemberReport', () => {
  ...
  it('renders the metadata-driven attendance-by-member table', async () => {
    // Arrange
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('attendance-by-member-report'),
      ).toBeInTheDocument(),
    );
    expect(screen.getByTestId('bifrost-table')).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { name: 'Attendance by Member' }),
    ).toBeInTheDocument();
  });

  it('queries event_attendance sorted by member_id', async () => {
    // Arrange
    globalThis.fetch = createFetchMock(identityWith(['main.members.read']));
    ...
    await waitFor(() => {
      ...
      expect(query!.query).toContain('member_id_asc');
    });
  });
