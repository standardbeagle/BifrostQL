...
export function DashboardPane({ fetcher, store = dashboardStore, onOpenTable }: { fetcher: GraphQLFetcher; store?: SavedObjectsClient; onOpenTable?: (table: string) => void }) {
  ...
  return <section className="bifrost-dashboard-pane" aria-label="Dashboards">
    ...
    <main className="bifrost-dashboard-main"><div className="bifrost-dashboard-toolbar"><label>Name <input aria-label="Dashboard name" value={name} onChange={(event) => setName(event.target.value)} /></label><button type="button" onClick={() => void save()}>Save dashboard</button>{active && <button type="button" onClick={() => void rename()}>Rename dashboard</button>}{active && <button type="button" onClick={() => void removeDashboard()}>Delete dashboard</button>}<button type="button" aria-pressed={editMode} onClick={() => setEditMode((value) => !value)}>{editMode ? "View dashboard" : "Edit dashboard"}</button>{editMode && <><button type="button" onClick={() => addTile("chart")}>Add chart</button><button type="button" onClick={() => addTile("count")}>Add count</button><button type="button" onClick={() => addTile("table")}>Add table</button></>}</div>
