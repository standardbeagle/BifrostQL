...
/// (default <c>_bifrost_saved_objects</c>, keyed by (owner, type, id)) provisioned on first
...
///
/// <para><c>owner</c> is the FIRST primary-key column and appears in the WHERE clause of every
/// read, update and delete, so no statement this class issues can span owners. It is supplied by
/// the caller from <see cref="SavedObjectOwner"/> and validated before it reaches a parameter.</para>
///
/// <para><b>A table provisioned before the owner column existed</b> will fail loudly on first use
/// (the column is missing) rather than silently reading rows nobody owns. There is no in-place
/// migration: the endpoint had not been wired into any host when the column was added, so no
/// deployment can hold rows in the old shape, and the recovery for a dev database is to drop the
/// table and let <c>CREATE TABLE IF NOT EXISTS</c> reprovision it.</para>
/// </summary>
public sealed class DbSavedObjectStore : ISavedObjectStore
{
    ...
    {
        new SqlColumnDefinition("owner", SqlColumnKind.Text, Nullable: false, PrimaryKey: true),
    ...
    public async Task<IReadOnlyList<SavedObject>> ListAsync(string owner, SavedObjectType? type, CancellationToken cancellationToken = default)
    {
        ...
        // The owner predicate is unconditional; only the type predicate is optional. A list that
        // could omit the owner is the unscoped query this store must not be able to express.
        var sql = $"SELECT {Col("id")}, {Col("type")}, {Col("name")}, {Col("folder")}, {Col("definition")}, {Col("version")}"
                + $" FROM {TableRef} WHERE {Col("owner")} = @owner";
        var parameters = new Dictionary<string, object?> { ["owner"] = SavedObjectOwner.Require(owner) };
        if (type.HasValue)
        {
            sql += $" AND {Col("type")} = @type";
            parameters["type"] = TypeValue(type.Value);
        }
    ...
    public async Task<SavedObject?> GetAsync(string owner, SavedObjectType type, string id, CancellationToken cancellationToken = default)
    {
        ...
        var sql = $"SELECT {Col("id")}, {Col("type")}, {Col("name")}, {Col("folder")}, {Col("definition")}, {Col("version")}"
                + $" FROM {TableRef} WHERE {Col("owner")} = @owner AND {Col("type")} = @type AND {Col("id")} = @id";
        var parameters = new Dictionary<string, object?>
        {
            ["owner"] = SavedObjectOwner.Require(owner), ["type"] = TypeValue(type), ["id"] = id,
        };
    ...
    public async Task<SavedObject> PutAsync(string owner, SavedObject obj, CancellationToken cancellationToken = default)
    {
        ...
        // Scoped to this owner, so an id another owner holds reads as absent and takes the create
        // path -- no conflict (which would leak that the id is taken) and no overwrite.
        var existing = await GetAsync(owner, obj.Type, obj.Id, cancellationToken);
        if (existing == null)
        {
            if (obj.Version != 0)
                throw new SavedObjectVersionConflictException(obj.Type, obj.Id, obj.Version, 0);
            return await InsertAsync(owner, obj, cancellationToken);
        }
        ...
        return await UpdateAsync(owner, obj, cancellationToken);
    }

    private async Task<SavedObject> InsertAsync(string owner, SavedObject obj, CancellationToken cancellationToken)
    {
        var sql = $"INSERT INTO {TableRef} ({Col("owner")}, {Col("type")}, {Col("id")}, {Col("name")}, {Col("folder")}, {Col("definition")}, {Col("version")})"
                + " VALUES (@owner, @type, @id, @name, @folder, @definition, @version)";
        var parameters = new Dictionary<string, object?>
        {
            ["owner"] = SavedObjectOwner.Require(owner),
        ...
        catch (Exception) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            var current = await GetAsync(owner, obj.Type, obj.Id, cancellationToken);
            if (current != null)
    ...
    private async Task<SavedObject> UpdateAsync(string owner, SavedObject obj, CancellationToken cancellationToken)
    {
        ...
        var sql = $"UPDATE {TableRef} SET {Col("name")} = @name, {Col("folder")} = @folder, {Col("definition")} = @definition, {Col("version")} = {Col("version")} + 1"
                + $" WHERE {Col("owner")} = @owner AND {Col("type")} = @type AND {Col("id")} = @id AND {Col("version")} = @expected";
        var parameters = new Dictionary<string, object?>
        {
            ["owner"] = SavedObjectOwner.Require(owner),
            ["type"] = TypeValue(obj.Type),
    ...
    public async Task DeleteAsync(string owner, SavedObjectType type, string id, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(id);
        await EnsureTableAsync(cancellationToken);

        var sql = $"DELETE FROM {TableRef} WHERE {Col("owner")} = @owner AND {Col("type")} = @type AND {Col("id")} = @id";
        var parameters = new Dictionary<string, object?>
        {
            ["owner"] = SavedObjectOwner.Require(owner), ["type"] = TypeValue(type), ["id"] = id,
        };
