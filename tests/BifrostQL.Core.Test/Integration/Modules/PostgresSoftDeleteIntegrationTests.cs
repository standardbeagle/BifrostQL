...
/// <summary>
/// PostgreSQL-specific soft delete integration tests.
/// Verifies SoftDeleteFilterTransformer and SoftDeleteMutationTransformer
/// produce correct PostgreSQL SQL syntax (double-quoted identifiers, LIMIT/OFFSET pagination).
/// Mirrors SoftDeleteIntegrationTests but uses PostgresDialect instead of SqlServerDialect.
/// </summary>
public class PostgresSoftDeleteIntegrationTests
{
    ...
    [Fact]
    public void SoftDeleteFilter_GeneratesPostgresIsNullWhereClause()
    {
        ...
        sql.Should().NotContain("[deleted_at]", "PostgreSQL uses double-quoted identifiers, not brackets");
    }

    [Fact]
    public void SoftDeleteFilter_CombinesWithUserFilter_PostgresSyntax()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
    ...
    [Fact]
    public void SoftDeleteFilter_WithTenantFilter_GeneratesBothFiltersInPostgres()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Orders", t => t
                ...
                .WithColumn("deleted_at", "timestamp", isNullable: true)
        ...
        sql.Should().Contain("\"deleted_at\"");
        sql.Should().Contain("IS NULL");
        sql.Should().NotContain("[", "PostgreSQL must not use bracket identifiers");
    ...
    public void SoftDeleteFilter_IncludeDeletedPerTable_OmitsFilterForSpecificTable()
    {
        ...
        var userContext = new Dictionary<string, object?> { ["include_deleted:public.Users"] = true };
        service.ApplyTransformers(query, model, userContext);
    ...
    public void SoftDeleteFilter_IncludeDeletedForOtherTable_StillFilters()
    {
        ...
        var userContext = new Dictionary<string, object?> { ["include_deleted:public.Orders"] = true };
        service.ApplyTransformers(query, model, userContext);
    ...
    #region PostgreSQL-Specific SQL Verification

    [Fact]
    public void PostgresSql_SoftDeleteFilter_UsesDoubleQuotedIdentifiers()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
    ...
    [Fact]
    public void PostgresSql_SoftDeleteFilter_UsesAtPrefixParameters()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
        ...
        {
            param.Name.Should().StartWith("@", "PostgreSQL parameters should use @ prefix");
            sql.Should().Contain(param.Name, "SQL should reference the parameter");
        }
    }

    [Fact]
    public void PostgresSql_SoftDeleteFilter_SchemaQualifiedTableReference()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Users", t => t
                .WithSchema("public")
        ...
        sql.Should().Contain("\"public\".\"Users\"", "should use schema-qualified table reference");
        sql.Should().Contain("\"deleted_at\"");
        sql.Should().Contain("IS NULL");
    }

    [Fact]
    public void PostgresSql_SoftDeleteFilter_LimitOffsetPagination()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
        ...
        sql.Should().NotContain("FETCH NEXT", "PostgreSQL uses LIMIT/OFFSET, not FETCH NEXT");
    }
    ...
    [Fact]
    public async Task FullSoftDeleteFlow_QueryExcludesDeleted_DeleteConvertsToUpdate_PostgresSql()
    {
        var model = CreateSoftDeleteWithDeletedByModel();
        var table = model.GetTableFromDbName("Users");

        // Part 1: Query filtering with PostgreSQL SQL generation
        ...
        selectSql.Should().NotContain("[", "PostgreSQL must not use bracket identifiers");
