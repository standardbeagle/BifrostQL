...
/// <summary>
/// SQLite-specific soft delete integration tests.
/// Verifies SoftDeleteFilterTransformer and SoftDeleteMutationTransformer
/// produce correct SQLite SQL syntax (double-quoted identifiers, LIMIT/OFFSET pagination).
/// Mirrors SoftDeleteIntegrationTests but uses SqliteDialect instead of SqlServerDialect.
/// </summary>
public class SqliteSoftDeleteIntegrationTests
{
    ...
    [Fact]
    public void SoftDeleteFilter_GeneratesSqliteIsNullWhereClause()
    {
        ...
        sql.Should().NotContain("[deleted_at]", "SQLite uses double-quoted identifiers, not brackets");
        sql.Should().NotContain("`deleted_at`", "SQLite uses double-quoted identifiers, not backticks");
    }

    [Fact]
    public void SoftDeleteFilter_CombinesWithUserFilter_SqliteSyntax()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
    ...
    [Fact]
    public void SoftDeleteFilter_WithTenantFilter_GeneratesBothFiltersInSqlite()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Orders", t => t
                ...
                .WithColumn("Total", "real")
                .WithColumn("deleted_at", "TEXT", isNullable: true)
        ...
        sql.Should().Contain("\"deleted_at\"");
        sql.Should().Contain("IS NULL");
        sql.Should().NotContain("[", "SQLite must not use bracket identifiers");
        sql.Should().NotContain("`tenant_id`", "SQLite must not use backtick identifiers");
    ...
    public void SoftDeleteFilter_IncludeDeletedPerTable_OmitsFilterForSpecificTable()
    {
        ...
        var userContext = new Dictionary<string, object?> { ["include_deleted:main.Users"] = true };
        service.ApplyTransformers(query, model, userContext);
    ...
    public void SoftDeleteFilter_IncludeDeletedForOtherTable_StillFilters()
    {
        ...
        var userContext = new Dictionary<string, object?> { ["include_deleted:main.Orders"] = true };
        service.ApplyTransformers(query, model, userContext);
    ...
    #region SQLite-Specific SQL Verification

    [Fact]
    public void SqliteSql_SoftDeleteFilter_UsesDoubleQuotedIdentifiers()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
        ...
        sql.Should().Contain("\"Users\"", "table name should use double quotes");
        sql.Should().Contain("\"Id\"", "column identifiers should use double quotes");
        sql.Should().Contain("\"deleted_at\"", "soft-delete column should use double quotes");
        ...
        sql.Should().NotContain("`Users`", "should not use MySQL backtick identifiers");
        sql.Should().NotContain("`Id`");
        sql.Should().NotContain("`deleted_at`");
    }

    [Fact]
    public void SqliteSql_SoftDeleteFilter_UsesAtPrefixParameters()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
        ...
        {
            param.Name.Should().StartWith("@", "SQLite parameters should use @ prefix");
            sql.Should().Contain(param.Name, "SQL should reference the parameter");
        }
    }

    [Fact]
    public void SqliteSql_SoftDeleteFilter_SchemaQualifiedTableReference()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Users", t => t
                .WithSchema("main")
        ...
        sql.Should().Contain("\"main\".\"Users\"", "should use schema-qualified table reference with double quotes");
        sql.Should().Contain("\"deleted_at\"");
        sql.Should().Contain("IS NULL");
    }

    [Fact]
    public void SqliteSql_SoftDeleteFilter_LimitOffsetPagination()
    {
        var model = CreateSoftDeleteModel();
        var table = model.GetTableFromDbName("Users");
        ...
        sql.Should().NotContain("FETCH NEXT", "SQLite uses LIMIT/OFFSET, not FETCH NEXT");
    }
    ...
    [Fact]
    public async Task FullSoftDeleteFlow_QueryExcludesDeleted_DeleteConvertsToUpdate_SqliteSql()
    {
        var model = CreateSoftDeleteWithDeletedByModel();
        var table = model.GetTableFromDbName("Users");

        // Part 1: Query filtering with SQLite SQL generation
        ...
        selectSql.Should().NotContain("[", "SQLite must not use bracket identifiers");
        selectSql.Should().NotContain("`deleted_at`", "SQLite must not use backtick identifiers");
    ...
    private static IDbModel CreateSoftDeleteWithDeletedByModel()
    {
        return DbModelTestFixture.Create()
            .WithTable("Users", t => t
                ...
                .WithColumn("Email", "TEXT")
                .WithColumn("deleted_at", "TEXT", isNullable: true)
                .WithColumn("deleted_by", "INTEGER", isNullable: true)
                .WithMetadata("soft-delete", "deleted_at")
