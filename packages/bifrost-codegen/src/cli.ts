...
import { emitGrants, emitSchema, type EmittedFile } from "./ts-emitter.js";
...
Usage:
  ...
  bifrostql-codegen --grants <path> --out <dir>

Options:
  --proto-file <path>    Path to a local .proto file (offline mode)
  --grants     <path>    Path to a GraphQL response containing _policyGrants
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  if (!opts.protoFile && !opts.endpoint && !opts.grantsFile) {
    process.stderr.write(`error: either --proto-file, --endpoint, or --grants must be provided\n\n${USAGE}`);
    return 2;
  }
  ...
  try {
    ...
    if (opts.grantsFile) {
      const response = JSON.parse(await readFile(resolve(opts.grantsFile), "utf8"));
      const policy = response.data?._policyGrants;
      const grants = response.data?._grants;
      if (Array.isArray(policy) && policy.length > 0 && (!Array.isArray(grants) || !grants.includes("admin"))) {
        process.stderr.write("warning: grant response may be narrowed; use admin credentials or a dedicated codegen identity\n");
      }
      files = [{ filename: "grants.ts", content: FILE_HEADER + "\n" + emitGrants(response) }];
    } else {
      const protoText = await loadProtoText(opts);
      const schema = parseProto(protoText);
      files = emitSchema(schema);
    }
