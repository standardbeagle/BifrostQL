...
/// <summary>
/// SQLite-specific tenant filtering integration tests.
/// Verifies TenantFilterTransformer produces correct SQLite SQL syntax
/// (double-quoted identifiers, LIMIT/OFFSET pagination, @-prefixed parameters).
/// Mirrors PostgresTenantFilterIntegrationTests but uses SqliteDialect instead of PostgresDialect.
/// </summary>
public class SqliteTenantFilterIntegrationTests
{
    ...
    [Fact]
    public void TenantFilter_GeneratesSqliteWhereClause()
    {
        ...
        sql.Should().Contain("\"tenant_id\"", "SQLite must double-quote the tenant column");
        sql.Should().NotContain("[tenant_id]", "SQLite must not use bracket identifiers");
    ...
    [Fact]
    public void TenantFilter_DoubleQuoteEscapesTableAndColumn()
    {
        var model = CreateTenantModel();
        var table = model.GetTableFromDbName("Orders");
        ...
        sql.Should().Contain("\"Orders\"", "table name must be double-quoted");
        sql.Should().Contain("\"tenant_id\"", "tenant column must be double-quoted");
        sql.Should().Contain("\"Id\"", "selected columns must be double-quoted");
        sql.Should().NotContain("[", "SQLite must not use bracket identifiers");
        sql.Should().NotContain("`", "SQLite must not use backtick identifiers");
    }
    ...
    public void TenantFilter_SchemaQualifiedTableReference()
    {
        ...
        sql.Should().Contain("\"main\".\"Orders\"", "should use schema-qualified table reference with double quotes");
    }
    ...
    [Fact]
    public void TenantFilter_WithSoftDelete_GeneratesBothFiltersInSqlite()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Orders", t => t
                .WithSchema("main")
    ...
    [Fact]
    public void TenantFilter_StringTenantId_GeneratesValidSqliteSql()
    {
        var model = DbModelTestFixture.Create()
            .WithTable("Orders", t => t
                .WithSchema("main")
    ...
    #region SQLite-Specific Parameter Verification
