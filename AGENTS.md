...
**Resolver 若自 selection set 導 SQL work，勿讀 `IResolveFieldContext.SubFields`。** `SubFields` 以 response key（alias）為鍵，每鍵唯存一 node：故一 schema field 若受二 alias（`s1: _sum {…} s2: _sum {…}`），則凡將 response key 折為 schema name 者（pre-M8 aggregate 之 `nodes[name] = sub.Field`，last-wins）唯得其末，餘 column 靜靜 serialise 為 null。且 `__typename` 亦列於 sub-fields 而非 column。正法：走 raw `context.FieldAst.SelectionSet`，解 inline 與 named fragment，每 schema name 存**全部** node 並 union 其 sub-fields，sub-field 名去重（重複 alias 令 result-set alias 相撞，`SqlExecutionManager` 之 `ToDictionary` 遂拋泛 DB error），`__` 前綴者跳過。Canonical walk 在 `Resolvers/SelectionWalk.cs`（`SelectedFieldNodes` / `SelectedSubFieldNames` / `Walk`）；`AggregateTableResolver` 與 `HistoryTableResolver` 皆引之，新 selection-derived resolver 亦然，勿另寫（私有 copy 已刪）。（`@skip`/`@include` 此 walk 不解，唯致 over-projection，不誤服 client。）一辨（`HistoryTrailSelectedColumnTests` 於 41a4d774 五 fact 皆綠所證）：execution engine 於 resolver 前已 merge **同** response key 之 selection set（spec CollectFields），故「flat + fragment spread」「flat + inline fragment」於 `SubFields` 內本為一 node 之 union，無 reader 因之丟欄；丟欄唯二者並存乃見：(a) 一 schema field 受 **≥2 distinct** response key（alias），且 (b) reader 以 schema name 為鍵折之（last-wins）。遍歷 `SubFields.Values` 而逐 node 收其 sub-selection 者（pre-change History）二 shape 皆不傷。然 (a) 之無傷乃 engine 之性，非 resolver 之性——故仍走 raw AST（raw `FieldAst.SelectionSet` **不** merge，故 `SelectionWalk` 之 last-node-only mutant 令 flat+fragment fact RED），勿恃之。History 之 sub-field 去重不在 walk 內，而在 `GqlObjectQuery.FullColumnNames` 之 `DistinctBy(alias)`。<!-- written_at: 2026-09-04T00:00:00Z  source_event: task:01M1KNYNEFYP8M0RC70387X4SM, git:87ea68c1 --><!-- amended_at: 2026-09-06T00:00:00Z  source_event: task:01M1QT7MGM129G7X6NNNMBFJNC, git:8fefb38f, git:d91d1bcb -->

**Read-path table identity 乃 `IDbTable`，非 bare DbName。** `DbName` 不足以identify 一 table：同名 table 得存於二 schema（`sales.orders` 與 `archive.orders`），而 `DbModel.GetTableFromDbName(string)` 於 ambiguous name 即 fail fast——故凡以 bare name 重解者，一 model 有此二 table，則其上之 query、join、filter、grouped aggregate、pivot 盡throw（M11）。今 `TableFilter.Table`、`GqlObjectQuery.DbTable`、`TableJoin.ConnectedTable.DbTable` 皆carry 已解之 `IDbTable`，一路thread 而下；`TableFilter.TableName` 唯 `Table?.DbName` 之 derived accessor，供 SQL rendering，不得作 lookup key。`TableFilter.FromObject` / `FromPrimaryKey` / `TableFilterFactory.*` 皆受 `IDbTable`，bare-name overload 已removed（非留為 fallback）。新 read surface 同thread `IDbTable`；唯 model-／config-derived 之名（其存在已知）方得用 throwing single-arg overload，client-reachable 之名必用 schema-qualified `GetTableFromDbName(schema, dbName)` 或其 `TryGet*`，且 miss 之 wire text 不載 name（見 `.claude/rules/protocol-adapter-security.md` invariant 3）。Write path（M11-w）：Core 之 wire-facing write entry point 受 client-supplied table 名者（`MutationIntent.Table` / `MutationBatchIntent.Table`——protocol adapters 經此寫者即承此規——與三 `File*Resolver` 之 `table` argument），唯一 resolution rule 乃 `IDbModel.TryGetTableFromClientName`——`schema.name` 精確解、bare name 唯全 model 唯一方解、ambiguous 與 unknown 同答一 sanitized `BifrostErrorSink.LookupMiss`（wire text byte-identical，不載名）；解得之 `IDbTable` 一路 thread 入五 keyed-write seams，seams 不自 re-derive。RESP／pgwire 之 read-side name resolution（`RespReadEngine`、`PgSubsetQueryTranslator.ResolveTable`）另有 adapter-owned index（DbName 或 GraphQlName 皆受，ambiguous 同 fail-fast），非此 rule；勿以此 rule 之 doc 推之。`ReadPathTableIdentityHygieneTests` 釘之（無 allowlist，掃 QueryModel/ 與 Resolvers/ 全部）。<!-- written_at: 2026-09-05T00:00:00Z  source_event: task:01M1KNYP1FA26W4H3R9GT4DKCB --><!-- amended_at: 2026-09-06T00:00:00Z  source_event: task:01M1THZTA5S39N7JKYFXJQ5YTW, git:7fc67d70 -->
...
**Keyed-write contract**：`TableMutationPipeline.SelectPredicateColumns` 統一選定 pre-chain predicate columns；`MutationArgumentBinder` 統一持有 `SplitKeyAndSet`、`RequireCompleteKey` 與 key/SET split；`MutationCommandExecutor` 統一持有 transformer-chain callers 的 keyed statement text 與 `EnsureAffectedRows` zero-row policy。四 keyed callers 為 per-row、batch、bulk plan、tree-sync；dialect bulk executors 僅消費 binder 決策所成之 plan。`FilteredUpdatePipeline` 是非 keyed 的 `TableFilter`-composed seam；`Storage/FilePointerAccess` 只是 per-row pipeline 的 caller，自身不建 keyed SQL（premise note 01M1MREARTJ84B5JGS8G46ANY8）。`KeyedWriteSourceScanTests` 以 transformer-chain caller 全檔掃描守此契約；並見 `.claude/rules/protocol-adapter-security.md` invariant 8。
...
- **Admission slot 必取於 ACCEPT**，先於 read、TLS handshake、authentication。cap 若後施，僅bound admitted sessions，不bound unauthenticated peer 所能forced 之work——非 cap。用 `ProtocolConnectionLimiter`，且每 adapter 自有 subtype（共用 base type 於 DI 則二 front doors 共一 counter）。acquire→deadline→run→release 之 lifecycle 今由 `ProtocolSessionHost` 擁（見下 bullet）：`RunAsync` 自取 slot，`RunAdmittedAsync` 予 listener middleware 已取者（RESP Kestrel path、LDAPS）——後者仍 arm deadline，勿因「已 admitted」而豁免之。
- **Pre-auth deadline 唯一在 `ProtocolSessionHost` / `ProtocolPreAuthDeadline`（`ProtocolSessionHost.cs`），必有，且唯 credentialed action 得退之。** slot 既取於 accept，silent peer 即 denial of service，無需credentials，無需bytes。authenticated 之後宜放寬或去之——idle authenticated session 乃 pooled client。然「retire」與「re-arm」異：deadline 若因某 free action（anonymous bind、unauthenticated ping、TLS renegotiate 之屬——即不經 rate limiter 者）而 reset，即成 renewable window，peer 逕自續命而無 credential，與無 deadline 等。故 `RetireOnCredentialedAction` 與 `ReArmOnReturnToAnonymous` 為二事：後者於 armed 時乃 no-op，free action 遂無從滑之；唯 credentialed→anonymous 之 transition 得 arm 一 fresh window（LDAP 失敗之 re-bind，RFC 4511 §4.2.1）。pgwire、RESP、LDAP 各曾自持一份 arm/retire 之 code，於二 clock seam 之上——H11(b)(c) 與 M17 即其 drift，且皆 fail OPEN。今一份：一 owned `new CancellationTokenSource(timeout, TimeProvider)` linked 於 connection token 而與 session 同壽（勿 detached `Task.Delay` loop），二 consumption shape 出於同一 rule——straight-line handshake 取其 `Token`（pgwire），message loop 取其 `ReadBudget`（RESP、LDAP：**shrinking** remainder，非每 read 一 fresh timeout；`clampToIdleWhileArmed` 唯 LDAP 為 true）。clock seam 乃 `TimeProvider`，非 `Func<DateTimeOffset>`——read timer 亦須經之，否則 injected clock 唯裝飾。新 raw-wire front door 同用之，勿另寫；`ProtocolPreAuthDeadlineUnificationTests` 之 whole-file source scan（錨於 pre-auth timeout option 之 read——copier 不可免者，非可改之 identifier）釘之。詳 `.claude/rules/protocol-adapter-security.md` invariant 15。<!-- amended_at: 2026-09-06T00:00:00Z  source_event: task:01M1KP3SDS29TYSRR6PCR6J31A -->
...
- **Raw-wire 之 identity projection唯一在 `AdapterIdentityProjection.TryProject`（`ProtocolAdapter.cs`）。** pgwire、RESP、LDAP 各曾自持一份 `TryProjectIdentity`——同一 fail-closed 決策之三份 copy：verified credential 唯證持 secret，未授 Bifrost identity；候選 principal 必經共享 `IBifrostAuthContextFactory` 投影，得 **non-empty** user context 方受之。empty context 非 refusal（invariant 12），僅 gate declare tenant metadata 之 table，餘者盡可讀——故三份 copy 即三處可 drift，且 drift 皆 fail OPEN。今一份，二 failure arm（projection 拋、projection 空）同返 false 與 empty out-param。新 raw-wire front door 同用之，勿另寫；`AdapterIdentityProjectionTests` 之 source scan（錨於 `CreateUserContext`，非可改之 method 名）釘之，`MountIdentityFunnelTests` 之 allowlist 亦隨之收窄。HTTP mount 另循 `BifrostIdentityGate.Project`（M13），二者不混。<!-- written_at: 2026-09-06T00:00:00Z  source_event: task:01M1KP3SDS29TYSRR6PCR6J31A -->
...
- **Untrusted input 之 regex 必 bounded**：`RegexOptions.NonBacktracking`（per-request scan 之正解，bound 在 engine，`Regex.InfiniteMatchTimeout`）或 match timeout；二者**不可互換**——hot path 上之 wall-clock timeout 於 host 負載時令合法 query 誤答 error（pgwire `PgCatalogResponder` 250 ms 即是，e44483f7），故 timeout 唯限非 per-request 之 cold path，且必 map 為 adapter 自有 exception type（見 `.claude/rules/protocol-adapter-security.md` invariant 1）。client-supplied pattern（LIKE 等）宜以 non-regex scan 行之。
...
| `ProtocolSessionHost` | `BifrostQL.Server/ProtocolSessionHost.cs` | Raw-wire admitted-session lifecycle: acquire → arm pre-auth deadline → run → release. One sealed subtype per front door (`PgwireSessionHost`, `RespSessionHost`, `LdapSessionHost`), each reading its own options — so the configured pre-auth timeout is read in exactly one file |
...
| `BifrostErrorSink` | `Resolvers/BifrostErrorSink.cs` | 「sanitize the wire, keep the detail」之唯一 seam：`LookupMiss(wireMessage, detail, site)` 以 Debug 記 caller-supplied name 於 server-side，返 sanitized `BifrostExecutionError`。process-wide static（`QueryField`、`BifrostDispatcher` 所建 resolvers 無 DI），`Logger` 由 HTTP middleware／intent executors 經 `BifrostErrorSink.Attach` 接——first writer wins，然其 publish 乃 `Interlocked.CompareExchange(ref _logger, logger, null)`，且 null candidate 逕不寫。前身之 `Logger ??= services?.GetService<ILogger>()` 乃 non-atomic read-then-write，而絕大多數 caller 無 services（candidate 為 null），故二 no-services constructor 併發時得於一 real logger 之 read 與 write 之間覆之為 null——測試所見之 flake 即此，非 ordering。新 attach site 必用 `Attach`，勿復 `??=`。凡 lookup miss 之 sanitized throw 必經此，勿另寫 log+throw 對 |<!-- amended_at: 2026-09-08T07:20:00Z  source_event: task:01M1ZDFZG7N0KX3Q03BXSE0M77, git:7463f3b4 -->
| `ProtocolAdapterConformanceTests` | `tests/BifrostQL.AdapterConformance/` | Derivable security-conformance kit; write adapters set `AdapterSupportsMutations`. Further opt-in flags: `AdapterSupportsAuthRateLimit`, `AdapterSupportsFrameLimit`, `AdapterSupportsContinuationTokens`, `AdapterSupportsMalformedFrameProbe`, `AdapterSupportsTlsAdmissionProbe`（admission slot 必先於 TLS handshake；derivation 須 bind 真 port，middleware registration order 非 in-process harness 所能觀） — each enables shared facts whose fixture hook the derivation supplies, so revert-prove per derivation |
