...
export function buildGridGroupingRequest(
    ...
    for (const name of [table.name, table.graphQlName, groupBy.graphQlName]) {
    ...
    return {
        query: `query GridGrouping${declarations} { ${table.name}Aggregate(${args}) { ${groupBy.graphQlName} _count } }`,
...
export function readGroupingRows(data: Record<string, unknown> | undefined, table: Table, groupBy: Column): GroupingRow[] {
    ...
    return rows.map((row) => {
        ...
        return { value: record[groupBy.graphQlName], count: Number(record._count ?? 0) };
    });
}
