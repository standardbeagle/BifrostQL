...
| `max-query-rows` | number | model | Server-side row ceiling for the GraphQL surface. Clamps the no-limit sentinel (`limit: -1`) and any explicit limit above it, on the root query, on nested collections (per parent for paged collections), and on the group window of `<table>Aggregate`. Default 10000; a non-positive value fails model load |
