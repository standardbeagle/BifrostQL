...
import { main, parseArgs, USAGE, writeFiles } from "./cli.js";
...
describe("grant catalogue beside the proto output", () => {
  const SAMPLE_PROTO = fileURLToPath(new URL("../fixtures/sample.proto", import.meta.url));
  const GRANT_BARREL_LINES = [
  ...
  it("emitSchema lands grants.ts and exactly two barrel lines when a catalogue is supplied", () => {
    const schema = parseProto(TWO_MESSAGE_SCHEMA);
    const files = emitSchema(schema, POLICY_GRANTS);

    const filenames = files.map((f) => f.filename).sort();
    expect(filenames).toEqual(
      ["OrderRow.ts", "Status.ts", "UserRow.ts", "grants.ts", "index.ts"].sort(),
    );

    const grants = files.find((f) => f.filename === "grants.ts")!;
    expect(grants.content.startsWith("// AUTO-GENERATED by @bifrostql/codegen")).toBe(true);
    expect(grants.content).toContain(emitGrants(POLICY_GRANTS));

    const index = files.find((f) => f.filename === "index.ts")!;
    const grantLines = index.content.split("\n").filter((line) => line.includes("grants.js"));
    expect(grantLines).toEqual(GRANT_BARREL_LINES);
    // The message/enum lines are untouched.
    expect(index.content).toContain('export type { UserRow } from "./UserRow.js";');
    expect(index.content).toContain('export { Status } from "./Status.js";');
  });

  it("emitSchema with an empty catalogue emits `never` and adds NO barrel line", () => {
    const schema = parseProto(TWO_MESSAGE_SCHEMA);
    const files = emitSchema(schema, { data: { _policyGrants: [], _grants: ["admin"] } });

    const grants = files.find((f) => f.filename === "grants.ts")!;
    expect(grants.content).toContain("export type Grant = never;");
    expect(grants.content).toContain("export const GRANTS: readonly Grant[] = [] as const;");

    const index = files.find((f) => f.filename === "index.ts")!;
    expect(index.content).not.toContain("grants.js");
  });

  async function runMain(args: readonly string[]): Promise<{ code: number; stderr: string }> {
    let stderr = "";
  ...
  async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
    const dir = await mkdtemp(join(tmpdir(), "bifrostql-codegen-grants-"));
    try {
      return await fn(dir);
    } finally {
      await rm(dir, { recursive: true, force: true });
  ...
  it("--proto-file + --grants writes grants.ts beside the messages and wires the barrel", async () => {
    await withTempDir(async (dir) => {
      const grantsFile = join(dir, "response.json");
      await writeFile(grantsFile, JSON.stringify(POLICY_GRANTS), "utf8");
      const outDir = join(dir, "generated");

      const { code, stderr } = await runMain([
        "--proto-file", SAMPLE_PROTO, "--grants", grantsFile, "--out", outDir,
      ]);
      expect(stderr).toBe("");
      expect(code).toBe(0);

      const grants = await readFile(join(outDir, "grants.ts"), "utf8");
      expect(grants).toContain("export type Grant = 'rates.view_cost' | 'team.manage';");
      const user = await readFile(join(outDir, "UserRow.ts"), "utf8");
      expect(user).toContain("export interface UserRow");
      const index = await readFile(join(outDir, "index.ts"), "utf8");
      for (const line of GRANT_BARREL_LINES) expect(index).toContain(line);
    });
  });

  it("warns on stderr (exit 0) when _policyGrants is non-empty but _grants lacks admin", async () => {
    await withTempDir(async (dir) => {
      const grantsFile = join(dir, "narrowed.json");
      await writeFile(
        grantsFile,
        JSON.stringify({ data: { _policyGrants: ["team.manage"], _grants: ["team.manage"] } }),
        "utf8",
      );
      const outDir = join(dir, "generated");
      ...
      expect(code).toBe(0);
      expect(stderr).toMatch(/^warning: .*narrowed/);
      // The narrowed catalogue is still emitted, beside the proto output.
      const grants = await readFile(join(outDir, "grants.ts"), "utf8");
      expect(grants).toContain("export type Grant = 'team.manage';");
      await readFile(join(outDir, "UserRow.ts"), "utf8");
    });
  });

  it("--grants without --proto-file or --endpoint fails naming the flag", async () => {
    await withTempDir(async (dir) => {
      const grantsFile = join(dir, "response.json");
      await writeFile(grantsFile, JSON.stringify(POLICY_GRANTS), "utf8");

      const { code, stderr } = await runMain(["--grants", grantsFile, "--out", join(dir, "gen")]);
      expect(code).toBe(2);
      expect(stderr).toMatch(/^error: --grants requires --proto-file or --endpoint/);
    });
  });
});
