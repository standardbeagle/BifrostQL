...
<DemoVideo
  scene="workbench-erd"
  title="The diagram the workbench draws for a connected database"
  description="Six tables laid out from the introspected schema: foreign keys, the posts–tags pair with its junction collapsed, an expanded column list, and the N-hop neighbourhood filter."
/>
