...
/**
 * Persistent, drag-resizable panel dimension.
 *
 * Returns the current size plus a pointerdown handler to put on a separator
 * element. Dragging follows the pointer along the given axis; the size is
 * clamped to [min, max] and persisted per `key` so a layout survives reload.
 * `invert` flips the drag direction for separators that sit BEFORE the panel
 * they size (e.g. a bottom panel resized by the handle above it: dragging the
 * handle UP must grow the panel).
 *
 * The separator also supports keyboard resizing (arrow keys, 16px steps) via
 * the returned onKeyDown — a drag-only separator is unreachable to keyboard
 * users. Callers should render the handle with role="separator" and an
 * aria-orientation matching the axis.
 */
...
const STORAGE_PREFIX = 'edit-db:panel:';
...
function saveSize(key: string, value: number): void {
    try {
        ...
        // Persistence is a nice-to-have; a blocked storage must not break resizing.
    }
}
...
export function usePanelSize({ key, initial, min, max, axis, invert = false }: PanelSizeOptions) {
    ...
    const onPointerDown = useCallback((e: React.PointerEvent<HTMLElement>) => {
        // Primary button / touch only; leave right-click menus alone.
