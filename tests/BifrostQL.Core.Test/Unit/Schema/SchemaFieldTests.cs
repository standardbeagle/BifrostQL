...
public class SchemaFieldSchemaGeneratorTests
{
    ...
    [Fact]
    public void FieldMode_DbColumnSchema_HasValidationFields()
    {
        var model = CreateMultiSchemaModel();
        var config = new SchemaFieldConfig
        {
        ...
        schema.Should().Contain("defaultValue: String");
        schema.Should().Contain("enumValues: [String!]");
        schema.Should().Contain("enumLabels: [String!]");
        // Verify metadata field is still present for extensibility
        schema.Should().Contain("metadata: [dbMetadataSchema!]!");
    }
