...
public sealed class SqliteDialectTest
{
    private readonly SqliteDialect _sut = SqliteDialect.Instance;
    ...
    {
        // Act
        var first = SqliteDialect.Instance;
        var second = SqliteDialect.Instance;
    ...
    {
        // Arrange
        const string schema = "attached_db";
        ...
        // Assert
        result.Should().Be("\"attached_db\".\"Orders\"");
    }
    ...
    {
        // SQLite rejects a bare `OFFSET n` with no preceding LIMIT (syntax error).
    ...
    [Fact]
    public void Pagination_LimitBeforeOffset_InSqliteConvention()
    {
        // SQLite convention: LIMIT comes before OFFSET
        // Arrange
    ...
    [Fact]
    public void LastInsertedIdentity_ReturnsLastInsertRowid()
    {
        // Act
        var result = _sut.LastInsertedIdentity;

        // Assert
        result.Should().Be("last_insert_rowid()");
    }
    ...
    public void BuildingSelectQuery_WithSchema_GeneratesValidSql()
    {
        // Arrange
        var dialect = SqliteDialect.Instance;
        var tableName = dialect.TableReference("main", "Users");
        var columnId = dialect.EscapeIdentifier("Id");
        var columnName = dialect.EscapeIdentifier("Name");
        ...
        // Assert
        sql.Should().Be("SELECT \"Id\", \"Name\" FROM \"main\".\"Users\"");
    }
    ...
    public void BuildingInsertQuery_WithIdentity_GeneratesValidSql()
    {
        ...
        // Assert
        sql.Should().Be("INSERT INTO \"Users\" (Name) VALUES (@p0); SELECT last_insert_rowid();");
    }
    ...
    public void AllInterfaceMethods_AreImplemented()
    {
        ...
        dialect.TableReference("main", "test").Should().NotBeNull();
    ...
    {
        // Verify SQLite uses double-quote identifiers, not SQL Server square brackets
    ...
    {
        // Verify SQLite uses LIMIT/OFFSET, not OFFSET/FETCH NEXT
    ...
    {
        // Verify SQLite uses || for string concatenation, not +
    ...
    [Fact]
    public void UsesLastInsertRowid_NotScopeIdentity()
    {
        // Verify SQLite uses last_insert_rowid(), not SCOPE_IDENTITY()
        _sut.LastInsertedIdentity.Should().Be("last_insert_rowid()");
        _sut.LastInsertedIdentity.Should().NotBe("SCOPE_IDENTITY()");
    }

    [Fact]
    public void UsesLastInsertRowid_NotLastval()
    {
        // Verify SQLite uses last_insert_rowid(), not PostgreSQL lastval()
        _sut.LastInsertedIdentity.Should().Be("last_insert_rowid()");
