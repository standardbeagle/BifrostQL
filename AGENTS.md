...
**Resolver 若自 selection set 導 SQL work，勿讀 `IResolveFieldContext.SubFields`。** `SubFields` 以 response key（alias）為鍵，每鍵唯存一 node：故一 schema field 若受二 alias（`s1: _sum {…} s2: _sum {…}`），或一次 flat 一次經 fragment spread，則以 schema name 查之者唯得其末，餘 column 靜靜 serialise 為 null。且 `__typename` 亦列於 sub-fields 而非 column。正法：走 raw `context.FieldAst.SelectionSet`，解 inline 與 named fragment，每 schema name 存**全部** node 並 union 其 sub-fields，sub-field 名去重（重複 alias 令 result-set alias 相撞，`SqlExecutionManager` 之 `ToDictionary` 遂拋泛 DB error），`__` 前綴者跳過。Canonical walk 在 `Resolvers/SelectionWalk.cs`（`SelectedFieldNodes` / `SelectedSubFieldNames` / `Walk`）；`AggregateTableResolver` 與 `HistoryTableResolver` 皆引之，新 selection-derived resolver 亦然，勿另寫（私有 copy 已刪）。（`@skip`/`@include` 此 walk 不解，唯致 over-projection，不誤服 client。）一辨：execution engine 於 resolver 前已 merge 同 response key 之 selection set，故「flat + fragment spread」之丟欄唯傷**以 schema name 查 `SubFields`** 者（M8 之 aggregate 即是）；遍歷 `SubFields.Values` 者不傷。然此乃 engine 之性，非 resolver 之性——故仍走 raw AST，勿恃之。<!-- written_at: 2026-09-04T00:00:00Z  source_event: task:01M1KNYNEFYP8M0RC70387X4SM, git:87ea68c1 --><!-- amended_at: 2026-09-06T00:00:00Z  source_event: task:01M1QT7MGM129G7X6NNNMBFJNC, git:8fefb38f, git:d91d1bcb -->
