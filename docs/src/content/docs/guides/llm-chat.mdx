...
<DemoVideo
  scene="chat-connectors"
  title="Chat over your database: explore, media, and approved writes"
  description="A real model reads pending orders through the explore connector, product photos stored as blobs render inline through the media route, and a plan-connector write lands only after the proposal card is approved."
/>
...
app.UseBifrostChat(chat =>
{
    ...
    chat.MaxMessageLength = 32768;   // default; chars per message, 400 when exceeded
...
| Status | Meaning |
|---|---|
| `400 invalid-request` | Malformed JSON, missing/blank `content`, or `content` longer than `MaxMessageLength`. |
...
| `413 payload-too-large` | The request body exceeded the cap (4 × `MaxMessageLength` + 1 KiB) — rejected before the body is materialized. |
...
Each individual message is bounded by **`MaxMessageLength`** (default 32768 characters): because up to `HistoryLimit` persisted messages ride every completion, an unbounded message would be an unbounded per-completion payload. Over-cap content is a `400` before anything is written, and the request body itself is capped at 4 × `MaxMessageLength` + 1 KiB (JSON envelope and UTF-8 expansion headroom) — over that is `413 payload-too-large` before the body is read into memory.
