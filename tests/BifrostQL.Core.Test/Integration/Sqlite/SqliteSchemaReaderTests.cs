...
public sealed class SqliteSchemaReaderTests : IAsyncLifetime
{
    ...
    private async Task CreateTestSchemaAsync()
    {
        var statements = new[]
        {
            ...
            @"CREATE TABLE ProductsWithUnique (
                Id INTEGER PRIMARY KEY AUTOINCREMENT,
                Sku TEXT NOT NULL,
                Name TEXT NOT NULL,
                CONSTRAINT UQ_Sku UNIQUE (Sku)
            )",
            @"CREATE TABLE MultiColumnUnique (
                Id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    [Fact]
    public async Task ReadSchema_ColumnWithUniqueConstraint_IsUniqueTrue()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);
        var table = schema.Tables.First(t => t.DbName == "Users");

        table.ColumnLookup["Username"].IsUnique.Should().BeTrue(
            "Column with UNIQUE constraint should be marked as unique");
        table.ColumnLookup["Email"].IsUnique.Should().BeTrue(
            "Column with UNIQUE constraint should be marked as unique");
    }

    [Fact]
    public async Task ReadSchema_ColumnWithoutUniqueConstraint_IsUniqueFalse()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);
        var table = schema.Tables.First(t => t.DbName == "Users");

        table.ColumnLookup["DisplayName"].IsUnique.Should().BeFalse(
            "Column without UNIQUE constraint should not be marked as unique");
    }

    [Fact]
    public async Task ReadSchema_PrimaryKeyColumn_IsUniqueFalse()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);
        var table = schema.Tables.First(t => t.DbName == "Users");

        // Primary keys should not be marked as unique (they have their own IsPrimaryKey flag)
        table.ColumnLookup["Id"].IsUnique.Should().BeFalse(
            "Primary key columns should not be marked as unique - they use IsPrimaryKey instead");
        table.ColumnLookup["Id"].IsPrimaryKey.Should().BeTrue();
    }

    [Fact]
    public async Task ReadSchema_TableWithNamedUniqueConstraint_Detected()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);
        var table = schema.Tables.First(t => t.DbName == "ProductsWithUnique");

        table.ColumnLookup["Sku"].IsUnique.Should().BeTrue(
            "Column with named UNIQUE constraint should be marked as unique");
    }

    [Fact]
    public async Task ReadSchema_MultiColumnUniqueConstraint_Detected()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);
        var table = schema.Tables.First(t => t.DbName == "MultiColumnUnique");

        // Both columns in the multi-column unique constraint should be marked as unique
        table.ColumnLookup["OrgId"].IsUnique.Should().BeTrue(
            "Column in multi-column UNIQUE constraint should be marked as unique");
        table.ColumnLookup["ExternalId"].IsUnique.Should().BeTrue(
            "Column in multi-column UNIQUE constraint should be marked as unique");
    }

    [Fact]
    public async Task ReadSchema_UniqueConstraintInColumnConstraints()
    {
        var schema = await _reader.ReadSchemaAsync(_connection);

        var usernameRef = new ColumnRef("main", "main", "Users", "Username");
        schema.ColumnConstraints.Should().ContainKey(usernameRef);

        var constraints = schema.ColumnConstraints[usernameRef];
        constraints.Should().Contain(c => c.ConstraintType == "UNIQUE",
            "UNIQUE constraint should be present in ColumnConstraints");
    }
