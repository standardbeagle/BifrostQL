...
import { UpcomingRenewalsReport } from './upcoming-renewals-report';
...
function renderReport() {
  return render(
    <PathProvider path="/reports/upcoming-renewals">
...
describe('UpcomingRenewalsReport', () => {
  ...
  it('renders the metadata-driven upcoming renewals table', async () => {
    // Arrange
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('upcoming-renewals-report'),
      ).toBeInTheDocument(),
    );
    expect(screen.getByTestId('bifrost-table')).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { name: 'Upcoming Renewals' }),
    ).toBeInTheDocument();
  });

  it('queries member_memberships filtered to a near-term end_date window', async () => {
    // Arrange
    globalThis.fetch = createFetchMock(identityWith(['main.members.read']));
    ...
    await waitFor(() => {
      ...
      expect(query!.query).toContain('end_date');
      expect(query!.query).toContain('_gte');
      expect(query!.query).toContain('_lte');
    });
  });
